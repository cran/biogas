...
\begin{table}[h!]
  \caption{Operations done with the high-level functions in the biogas package. The \texttt{cumBg()} and \texttt{summBg()} functions can handle data from any number of reactors. \texttt{predBg()} is vectorized.}
...
As of bigoas version 1.5.0, the "long" data structures described above are not the other options.
...
So we see that theoretical methane potential of cellulose is 414 \si{mL.g^{-1}}. 
...
We can conclude that our measured results are reasonable.
