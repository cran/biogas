...
The \texttt{fCH4} value is $\approx 55\%$ in our example, which is not the same as the \ce{CH4} fraction in biogas.
...
\begin{equation*}
  \ce{
    C_nH_aO_bN_c + (2n + c - b - \frac{9df_s}{20} - \frac{9f_e}{4})H2O -> 
  }
\end{equation*}
