...
%\VignetteIndexEntry{Using biogas}
...
%\usepackage{fullpage} % put to full page 
\usepackage{listings} % to break lines in R chuncks os it does not go out of the box
%\usepackage{natbib} % bibliography % NTS do not use, causes problems with citation format
\usepackage{hyperref} 
\usepackage[version = 3]{mhchem} %chemical formulas
\usepackage{siunitx} % to write more efficiently units and numbers
\usepackage{parskip} % Space instead of indent
\usepackage{tabularx} % to adjust automatically column size 
\usepackage{booktabs} % to use top and bottom in tables in bolds
...
\author{Charlotte Rennuit (\texttt{cre@kbm.sdu.dk}) and Sasha D. Hafner (\texttt{saha@kbm.sdu.dk})}
...
Transformation of raw data collected in laboratory experiments into quantities and rates of methane (\ce{CH4}) production requires a sequence of simple calculations.
...
The package includes six ``low-level'' functions (Table \ref{tab:lowfunctionsummary}) and three ``high-level'' functions (Table \ref{tab:highfunctionsummary}).
To go from data collected in the laboratory to biogas and methane (\ce{CH4}) production or biochemical methane potential (BMP), two high-level functions are needed: \texttt{cumBg()} and \texttt{summBg()}.
...
\begin{table}[h!]
  \begin{center}
  \caption{Operations done with the low-level functions in the biogas package. All functions except \texttt{calcCOD()} and \texttt{molMass()} are vectorized. See help files for more details.}
...
\begin{table}[h!]
  \caption{Operations done with the high-level functions in the biogas package. The \texttt{cumBg()} and \texttt{summBg()} functions can handle data frames. \texttt{predBg()} is vectorized.}
  ...
  \begin{tabular}{ll}
    ...
    Operation                                                                          & Function            \\
    \hline
    Calculate cumulative \ce{CH4} production and rates from volume (mass), composition & \texttt{cumBg()}      \\
...
Calculation of biochemical methane potential (BMP) typically requires three data frames: initial mass, biogas volume (or reactor mass loss), and biogas composition.
Volume and composition data frames must be in a ``long'' format, with the measured variable in a single column (Fig. \ref{fig:timedata}).
Columns with unique reactor IDs and time allow the \texttt{biogas} functions to link observations in the two data frames\footnote{
...
It is possible to put volume and composition data in a single data frame, but since composition is not always measured concurrently with volume, a better approach is to use two separate data frames.
...
It should contain at least a reactor ID column and a description of the reactor contents.
...
Any measurements to be used to normalise biogas or \ce{CH4} production are included here, using a ``wide'' format (Fig. \ref{fig:inidata}).
...
These data are from a BMP test that was carried out on two different susbtrates A and B, and cellulose (included as a ``control'').
The experiment included 12 batch reactors:
\begin{itemize}
  \item 3 reactors with substrate A and inoculum
  \item 3 reactors with substrate B and inoculum
  \item 3 reactors with cellulose and inoculum
  \item 3 reactors with inoculum only
\end{itemize}
...
The first step in processing these data is to calculate cumulative production of biogas and \ce{CH4} and production rates.
...
args(cumBg)\end{lstlisting}
\begin{lstlisting}[basicstyle = \ttfamily, breaklines = true]
## function (dat, dat.type = "vol", comp = NULL, temp = NULL, 
##     pres = NULL, id.name = "id", time.name = "time", 
##     dat.name = dat.type, comp.name = "xCH4", headspace = NULL, 
##     vol.hs.name = "vol.hs", headcomp = "N2", temp.init = NULL, 
##     temp.std = getOption("temp.std", 0), 
##     pres.std = getOption("pres.std", 1), 
##     unit.temp = getOption("unit.temp", "C"), 
##     unit.pres = getOption("unit.pres", "atm"), 
##     cmethod = "removed", imethod = "linear", extrap = FALSE, 
##     addt0 = TRUE, showt0 = TRUE, std.message = TRUE, 
##     check = TRUE) 
## NULL
\end{lstlisting}
\end{kframe}
...
Most of the arguments have default values, but to calculate \ce{CH4} production we must provide values for at least \texttt{dat} (we will use \texttt{vol}), \texttt{comp} (we will use \texttt{comp}), \texttt{temp} (biogas temperature), and \texttt{pres} (biogas pressure)\footnote{
...
Similarly, there is an \texttt{id.name} argument for the reactor ID column (used to match up volume and composition data), but we can use the default value (\texttt{"id"}) here because it matches the column name in \texttt{vol} and \texttt{comp}.
And, the \texttt{comp.name} argument is used to indicate which column within the \texttt{comp} data frame contains the \ce{CH4} content (as mole fraction in dry biogas, normalised so the sum of mole fractions of \ce{CH4} and \ce{CO2} sum to unity).
...
By default (\texttt{cmethod = "removed"}) the function calculates volumes following \cite{richards_methods_1991} as the product of standardised volume of biogas removed and normalised \ce{CH4} content. 
...
So \texttt{cvCH4} contains standardised cumulative \ce{CH4} production.
...
To calculate BMP we need to substract the contribution of the inoculum to \ce{CH4} production for each reactor, normalise by substrate volatile solids (VS), and calculate means and standard deviations.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{lstlisting}[basicstyle = \ttfamily, breaklines = true]
args(summBg)\end{lstlisting}
\begin{lstlisting}[basicstyle = \ttfamily, breaklines = true]
## function (vol, setup, id.name = "id", time.name = "time", 
##     descrip.name = "descrip", inoc.name = NULL, norm.name = NULL, 
##     inoc.m.name = "minoc", vol.name = "cvCH4", 
...
For example, to simply determine the mean cumulative \ce{CH4} production for each substrate at 30 d, we could use:
...
Here, the response variable was \texttt{cvCH4} (cumulative \ce{CH4} production, the default--but \texttt{vol.name} could be used to specify any column).
The argument \texttt{descrip.name} is the name of the column in \texttt{setup} that gives a description of the reactor.
Here it is used for grouping reactors.
...
To subtract the inoculum contribution, we need to provide a value for the \texttt{inoc.name} argument, which should be the value in the \texttt{setup\$descrip.name} column that indicates that the reactor contained inoculum only.
...
The last step is normalisation of cumulative \ce{CH4} production, based on substrate VS mass.
...
We will evaluate \ce{CH4} production at 60 days (\texttt{when} argument).
...
Additionally, it is good practice to view and save results from individual reactors, and check the apparent contribution of the inoculum to each reactor's biogas production.
...
Using its chemical formula (\ce{C6H10O5}), we can calculate theoretical methane potential to compare to our measurements\footnote{
  In this case, the calculation is based on Eq. (13.5) in Rittmann and McCarty \cite{rittmann_environmental_2001}.
  When the input is COD, it is based on the COD of \ce{CH4}, as described in \cite{rittmann_environmental_2001}.
}.
...
So we see that theoretical methane potential of cellulose is \Sexpr{round(predBg("C6H10O5"), 0)} \si{mL.g^{-1}}. 
...
Initial COD masses are in the \texttt{setup} data frame, and from these we can calculate COD:VS ratios for substrates A and B of 1.439 and 1.561 \si{g.g^{-1}}. 
Cellulose has a calculated oxygen demand (COD$'$)\footnote{
  Oxygen demand can be calculated with the \texttt{calcCOD} function.  
}
 of 1.184 \si{g.g^{-1}}.
Predicted \ce{CH4} production per g VS is therefore:
...
In fact, we could use \texttt{predBg()} to estimate degradability.
...
We can conclude that are measured results are reasonable.
...
For example, \texttt{cumBg()} can be used with measurements of reactor mass over time to determine biogas production\cite{hafner_validation_2015}, \texttt{summBg()} can return results for multiple times, and \texttt{predBg()} function can predict microbial nitrogen requirements and biogas composition.
More details can be found in the help files for these functions.
...
Sasha~D. Hafner, Charlotte Rennuit, Jin~Mi Triolo, and Brian~K. Richards.
\newblock Validation of a simple gravimetric method for measuring biogas
  production in laboratory experiments.
\newblock {\em Biomass and Bioenergy}, In review.

\bibitem{richards_methods_1991}
BK~Richards, RJ~Cummings, TE~White, and WJ~Jewell.
