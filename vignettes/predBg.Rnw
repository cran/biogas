...
\author{Sasha D. Hafner (\texttt{saha@kbm.sdu.dk}) and Charlotte Rennuit (\texttt{cre@kbm.sdu.dk})}
...
Cellulose is often included in biochemical methane potential (BMP) experiments as a positive control--a comparison between the measured and expected quantity of \ce{CH4} produced is used to evaluate the accuracy of the measurements.
How much \ce{CH4} should we expect from cellulose?
...
So if it were all converted to \ce{CH4}, we should see about 414 mL of \ce{CH4} (dry, at 101.325 kPa and 0$^\circ$C)\footnote{
...
Continuing with our example, to predict \ce{CH4} from 5.75 g of cellulose, set \texttt{mass = 5.75}.

<<cellulose2>>=
  predBg("C6H10O5", mass = 5.75)
@

In this case we should expect about 2400 mL of \ce{CH4}.
...
But even for a substrate that is completely consumed by the bacteria and archaea in an anaerobic reactor (like microcrystaline cellulose) not all of the substrate is converted to \ce{CH4}.
...
We can conclude that \ce{CH4} production from our 5.75 g of cellulose should be at least 1900 mL.
...
We can start by comparing theoretical \ce{CH4} production from each substrate on  mass and molar bases.
...
From these results we can see that \ce{CH4} production increases with the molecular mass of the acid, on both a mass and mole basis\footnote{
...
For a particular assay, assume we plan on using 60 mL of medium containing formic acid:acetic acid at 1:1 (mole basis) in a 100 mL serum bottle, and need at least 5 mL of \ce{CH4} for accurate quantification.
We can specify predict \ce{CH4} production from such a mixture using the notation for \texttt{form} shown below.
...
Let's double-check to make sure it would provide a maximum of 50 mL of \ce{CH4}.
...
The response variable that we are interested in (and the one we worked with above) is the predicted \ce{CH4} volume \texttt{vCH4}, in mL.
The remaining columns show hydrolytic water consumption (\texttt{hydro}, g of \ce{H2O}) (in this example, \ce{H2O} is produced, not consumed), the molar fraction of \ce{CH4} in the reaction products (\ce{CH4} + \ce{CO2}) (\texttt{fCH4}), and the masses of of \ce{CH4} and \ce{CO2} produced (\texttt{mCH4} and \texttt{mCO2}, g).
...
With these parameters, biomass production (\texttt{m.bio}, g) and the reduced N requirement (\ce{NH4+}) for biomass production (\texttt{N.req}, g) can be calculate and are returned.
...
So we can conclude that we need at least 31 mg L$^{-1}$ of \ce{NH4+} in our medium.

\section{Unknown composition: using COD}
If the composition of a substrate is not known, it is still possible to predict \ce{CH4} production based on a measured chemical oxygen demand (COD).
For example, assume we have a waste stream with a COD of 2600 mg L$^{-1}$.
What is the maximum volume of \ce{CH4} that could be produced?

<<cod1>>=
  predBg(COD = 2.6)
@

We should expect no more than 908 mL \ce{CH4}, and probably much less, depending on the degradability of our substrate.
...
For example, we could take the empirical formula calculated above, \ce{C_{29.2587}H_{46.984}O_{18.9464}N_1}, round it for simplicity to \ce{C_{29.3}H_{47}O_{18.9}N} and use it as the \texttt{form} argument.
...
We can see that predicted \ce{CH4} production per g of (mixed) substrate increases substantially when crude glycerol is added.
...
Predicting total biogas production is complicated by the behavior of \ce{CO2} after it is produced.
Unlike \ce{CH4}, which is only sparingly soluble in water, a significant fraction of \ce{CO2} produced remains in solution, and therefore does not contribute to biogas volume or affect its composition.
In the examples we've covered so far, we could get the fraction of \ce{CH4} in the reaction products \texttt{fCH4} by setting the \texttt{value} argument to \texttt{"all"}.
...
Only after specifying values for these additional arguments will \texttt{predBg} return a column with biogas composition: \texttt{xCH4} gives the mole fraction of \ce{CH4} in dry biogas. 
...
Additional new columns in this output give data for the volume of \ce{CO2} in biogas (\texttt{vCO2}, mL), the total volume of (dry, standardized) biogas (\texttt{vBg}, mL), the mass of \ce{CO2} in biogas and in solution (\texttt{mCO2Bg} and \texttt{mCO2.sol}, g), and the total inorganic carbon concentration in solution (\texttt{cTIC}, mol kg$^{-1}$).
In fact, nearly one-third of all the \ce{CO2} produced ends up in the solution in this example--it may be higher in other cases.
...
The method assumes that all \ce{CO2} in the system is accounted by the empirical formula (\texttt{form}).
This may not be the case--for example if a bicarbonate (\ce{HCO3-}) salt is included as a buffer in our BMP test with cellulose, or for our manure example, there is certainly significant TIC in raw manure.
...
%@
%
%To predict \ce{CO2} partitioning, we need to add values from several parameters.
%
%<<manurepart2>>=
%  predBg('C29H47O19N', mass = 1, fd = 0.4, fs = 0.1, conc.sub = 40, pH = c(7, 7.5, 8), temp = 35, value = "all")
%@
%
%The function predicts a relatively high \ce{CH4} content at the highest pH: 86\%!
...
\begin{equation*}
  \ce{
    C_nHO_bN_c + (2n + c - b - \frac{9df_s}{20} - \frac{9f_e}{4})H2O -> 
  }
\end{equation*}

\begin{equation*}
  \ce{
    \frac{df_e}{8}CH4 + (n - c - \frac{df_s}{5} - \frac{df_e}{8})CO2 + \frac{df_s}{20}C5H7O2N
  }
\end{equation*}

\begin{equation*}
  \ce{
    + (c - \frac{df_s}{20})NH4+ + (c - \frac{df_s}{20})HCO3-
  }
\end{equation*}

Here, $f_s$ and $f_e$ are the fraction of substrate electrons going to cell synthesis and energy production, respectively, the formula \ce{C5H7O2N} is an empirical formula for cell biomass\footnote{
  Presently it is not possible to use a different formula.
}, and $d= 4n + a -2b - 3c$.
In addition to providing estimates of \ce{CH4} production, this approach allows \texttt{predBg()} to return production of \ce{CO2}\footnote{
  More accurately, total inorganic carbon (TIC), which includes both \ce{CO2} and \ce{HCO3-} from the above reaction.
}, consumption or production of ammonia, and production of cell biomass.
Predicted \ce{CH4} is always expressed in mL (cm$^3$) at standard conditions of 101.325 kPa (1.0 atm) and 0$^\circ$C (273.15 K).
...
For acetate fermenting methanogens, $f_s^0 = 0.05$, and for hydrogen oxidizer methanogens, $f_s^0 = 0.08$.
...
The \texttt{predBg()} function can also calculate \ce{CH4} production from COD.
In this case, \ce{CH4} volume is calculated based on the oxidation of \ce{CH4} with \ce{O2}\footnote{
This reaction is based on a single electron equivalent.  
}\cite{rittmann_environmental_2001}:

\begin{equation*}
  \ce{1/8CH4 + 1/4O2 -> 1/8CO2 + 1/4H2O }
\end{equation*}

which gives a ratio of \ce{CH4} to \ce{O2} of $\ce{1/8}:\ce{1/4} = 1:2$.
Based on conservation of COD, potential \ce{CH4} production of a substrate with COD $c$ is therefore:
...
where $v_{CH_4}=$ molar volume of \ce{CH4} (22361 mL g$^{-1}$, defined at 1 atm and 0$^\circ$C \cite{linstrom_nist_2011}) and $M_{O_2}$ is the molar mass of \ce{O2} (32.0 g mol$^{-1}$).
...
The approach used to calculate \ce{CO2} partitioning is based on equilibrium speciation as described in \cite{hafner_role_2012} and should be accurate for continuous reactors with stable operation at least.
...
Sasha~D. Hafner, Felipe Montes, and C.~Alan~Rotz.
